"""
Modular ML models for men's T20 cricket prediction.

This module provides:
- BasePredictor: Common functionality for MLflow tracking and model management
- RunsPredictor: LinearRegression for total runs prediction  
- WicketPredictor: LogisticRegression for wicket probability prediction
"""
...
class BasePredictor(ABC):
    ...
    def __init__(
        ...
        logger.info(f"Initialized {self.__class__.__name__} for experiment: {experiment_name}")
    ...
    def train(
        ...
        with mlflow.start_run() as run:
            ...
            if X_val is not None and y_val is not None:
                ...
                key_metric = "r2" if hasattr(self, "_is_regressor") and self._is_regressor else "accuracy"
                if f"train_{key_metric}" in train_metrics and f"val_{key_metric}" in val_metrics:
                    mlflow.log_metric(
                        f"val_train_{key_metric}_diff",
                        train_metrics[f"train_{key_metric}"] - val_metrics[f"val_{key_metric}"]
                    )
    ...
    def _get_model_params(self, X_train: np.ndarray, X_val: Optional[np.ndarray]) -> Dict[str, Any]:
...
class RunsPredictor(BasePredictor):
    ...
    def __init__(self, experiment_name: str, model_name: str, random_state: Optional[int] = 42):
...
class WicketPredictor(BasePredictor):
    ...
    def _calculate_metrics(self, y_true: np.ndarray, y_pred: np.ndarray, prefix: str = "") -> Dict[str, float]:
        ...
        if y_pred.ndim > 1 or (y_pred.dtype == float and ((y_pred >= 0) & (y_pred <= 1)).all()):
        ...
        metrics = {
            ...
            f"{prefix}_precision": precision_score(y_true, y_pred_binary, zero_division=0),
        ...
        try:
            if hasattr(self.model, 'predict_proba') and self._is_trained:
                ...
                y_prob = self.model.predict_proba(
                    X=getattr(self, '_last_X', None) if hasattr(self, '_last_X') else None
                )
...
def create_runs_predictor(experiment_name: str = None, model_name: str = None) -> RunsPredictor:
...
def create_wicket_predictor(experiment_name: str = None, model_name: str = None) -> WicketPredictor:
